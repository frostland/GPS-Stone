...
== Notes
We currently have included a `UIRequiredDeviceCapabilities` with a `location-services` value
 (`gps` is too much, I think: we don’t need GPS accuracy to be able to record something).
